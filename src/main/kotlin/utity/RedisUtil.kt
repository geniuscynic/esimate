 ...
 * @date 2021/8/3
 */
class RedisUtil(val redisTemplate: RedisTemplate<String, String>) {
    //@Autowired
    ...
    inline operator fun <reified T> get(key: String): T? {
        return this.redisTemplate.opsForValue()[key]?.let { result ->
            return when (T::class.java) {
                java.lang.Integer::class.java -> result as T
                java.lang.Long::class.java -> result as T
                java.lang.String::class.java -> result as T
                java.lang.Boolean::class.java -> result as T
                else -> JSON.parseObject(result, T::class.java)
            }
        }
    }
     ...
     */
    operator fun set(key: String, value: String, time: Long): Boolean {
        return try {
            if (time > 0) {
     ...
     * @return 成功个数
     */
    fun sSetAndTime(key: String, time: Long, vararg values: String): Long? {
        return try {
     ...
     * @return
     */
    fun lSet(key: String, value: String): Boolean {
        return try {
     ...
     * @return
     */
    fun lSet(key: String, value: String, time: Long): Boolean {
        return try {
            redisTemplate.opsForList().rightPush(key, value)
            if (time > 0) expire(key, time)
            true
     ...
     * @return
     */
//    fun<V> lSet(key: String, value: List<V>): Boolean? {
//        return try {
...
//    }
     ...
     * @return
     */
    fun lUpdateIndex(key: String, index: Long, value: String): Boolean {
        return try {
     ...
     * @return 移除的个数
     */
    fun lRemove(key: String, count: Long, value: String): Long? {
        return try {
